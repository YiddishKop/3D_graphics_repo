...
function createStairs() {
	...
	// one step consist of 1 horizontalStep and 1 verticalStep
	for (var stepNum = 1; stepNum * (verticalStepHeight + stepThickness) < cupHeight; stepNum++)
	{
		...
		stepMesh.position.y = verticalStepHeight/2 + (stepNum - 1) * (verticalStepHeight + stepThickness);	// half of height: put it above ground plane
		stepMesh.position.z = (stepNum - 1) * (horizontalStepDepth - stepThickness);			// centered at origin
		scene.add( stepMesh );
		...
		stepMesh.position.y = stepHalfThickness + verticalStepHeight + (stepNum - 1) * ( verticalStepHeight + stepThickness);
		// Push step forward by half the depth, minus half the vertical step's thickness
		stepMesh.position.z = horizontalStepDepth/2 - stepHalfThickness + (stepNum - 1) * (horizontalStepDepth - stepThickness);

		// re-compute the stepsHeight, used for verify whether steps reach the cup
